...
export default function CharactersList({ characters }: Props) {
  ...
  return (
    <ul title='characters' className='grid gap-16 grid-cols-[repeat(auto-fill,minmax(220px,1fr))]'>
      {characters.map((character) => (
        <li key={character.id} className='rounded-12 bg-gray-900 focus-within:bg-gray-800'>
          <Link
          ...
          >
            ...
            <div className='p-16 text-white flex flex-col gap-4 grow'>
              ...
              <p className='font-medium text-gray-200 text-body3'>{character.species}</p>
